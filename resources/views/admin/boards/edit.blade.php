<x-layouts::app :title="__('Crear Board')">
    ...
    <div class="card">
        <form action="{{ route('admin.boards.update', $board->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <flux:input label="Nombre del Board" name="name" value="{{ old('name', $board->name) }}" placeholder="Escribe el nombre del board" />
